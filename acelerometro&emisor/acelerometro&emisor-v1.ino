...
uint8_t direcciones[NUM_SENSORES] = {0x68,
                                     0x69};  // direcciones I2C de los sensores
...
void setup() {
  ...
  mrf.address16_write(0x6001);  // Dirección del emisor (AD1 o AD2)
  mrf.set_promiscuous(true);
  debug("Transmisor MRF iniciado.");
...
void enviar_datos() {
  ...
  msg += " | Velocidad (km/h)=" + String(v_total * 3.6, 2);
  debug(msg);

  // Crear JSON para transmitir
  StaticJsonDocument<200> doc;
  doc["id"] = "AD1";  // puedes cambiarlo a AD2 si es otro emisor

  JsonObject a = doc.createNestedObject("a");
  a["x"] = datos[NUM_SENSORES].ax;
  a["y"] = datos[NUM_SENSORES].ay;
  a["z"] = datos[NUM_SENSORES].az;
  ...
  mrf.send16(0x6003, buffer);  // 0x6003 es el receptor

  debug("JSON enviado: " + String(buffer));
}
